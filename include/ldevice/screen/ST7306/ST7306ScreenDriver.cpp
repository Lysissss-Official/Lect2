...
ST7306ScreenDriver::ST7306ScreenDriver(
    const Config& config
) : config_(config) {
...
bool ST7306ScreenDriver::initScreenCmd() {
    if (initialized_) {
        return true;
    }
...
bool ST7306ScreenDriver::closeScreenCmd() {
    if (!spi_device_) {
        initialized_ = false;
        return true;
    }
...
bool ST7306ScreenDriver::configurePins() {
    if (
...
bool ST7306ScreenDriver::initBus() {
    if (spi_device_) {
        return true;
    }
...
void ST7306ScreenDriver::closeBus() {
    if (spi_device_) {
...
void ST7306ScreenDriver::hardwareReset() {
...
bool ST7306ScreenDriver::transmit(
    bool data_mode,
    const void* data,
    std::size_t size,
    uint32_t flags
...
bool ST7306ScreenDriver::sendCommand(
    uint8_t command,
    const uint8_t* data,
...
bool ST7306ScreenDriver::initializePanel() {
...
uint8_t ST7306ScreenDriver::colorToPanelBits(
    uint32_t color
) {
...
uint8_t ST7306ScreenDriver::packedPairForColor(
    uint32_t color
) {
...
uint8_t& ST7306ScreenDriver::pixelPairByte(
    uint16_t x,
    uint16_t y
) {
...
bool ST7306ScreenDriver::drawPixelCmd(
    uint16_t x,
    uint16_t y,
    uint32_t color
) {
...
bool ST7306ScreenDriver::ClearScreenCmd() {
...
bool ST7306ScreenDriver::drawLineCmd(
    uint16_t x1,
    uint16_t y1,
    uint16_t x2,
    uint16_t y2,
...
bool ST7306ScreenDriver::drawFilledRectCmd(
    uint16_t x1,
    uint16_t y1,
    uint16_t x2,
    uint16_t y2,
...
bool ST7306ScreenDriver::refreshScreenCmd() {
    if (!spi_device_) {
        return false;
    }
